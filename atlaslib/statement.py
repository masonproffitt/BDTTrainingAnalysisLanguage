...
class loop(block):
    ...
    def emit (self, e):
        ...
        e.add_line("for (auto {0} : {1})".format(self._loop_variable, self._collection_name))
        block.emit(self, e)
...
class book_ttree:
    ...
    def emit (self, e):
        ...
        e.add_line ('ANA_CHECK (book (TTree ("{0}", "My analysis ntuple")));'.format(self._tree_name))
        e.add_line ('auto mytree = tree ("{0}");'.format(self._tree_name))
        for var_pair in self._leaves:
            e.add_line ('mytree->Branch("{0}", &{1});'.format(var_pair[0], var_pair[1]))
...
class xaod_get_collection:
    ...
    def emit (self, e):
        ...
        e.add_line('ANA_CHECK (evtStore()->retrieve( {0}, "{1}"));'.format(self._var_name, self._collection_name))
